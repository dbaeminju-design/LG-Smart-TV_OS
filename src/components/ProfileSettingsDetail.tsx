...
// ─── 컴포넌트 ─────────────────────────────────────────────────────────────────
export function ProfileSettingsDetail({ profile, onSave, onCancel, onUpdateTimeLimit }: Props) {
  const theme = getThemeByAge(profile.age)

  // 로컬 설정 상태
  const [timeLimit,   setTimeLimit]  = useState(profile.timeLimit)
  const [dayLimits,   setDayLimits]  = useState<Record<string, number>>(
    Object.fromEntries(
      DAYS.map(d => [d, d === '토' || d === '일' ? Math.min(profile.timeLimit + 20, 240) : profile.timeLimit])
    )
  )
  const [ratingIdx,   setRatingIdx]  = useState(0)
  const [tooltipIdx,  setTooltipIdx] = useState<number | null>(null)
  const [dayOpen,     setDayOpen]    = useState(false)
  const [bedtimeOn,   setBedtime]    = useState(false)
  const [bedtimeHour, setBedHour]    = useState(21)
  const [protectOn,   setProtect]    = useState(true)
  ...
  }, [timeLimit, ratingIdx, bedtimeOn, protectOn, camFeatures, profile.timeLimit])
  ...
  const handleSave = useCallback(() => {
    ...
    onSave({ timeLimit })
    setIsDirty(false)
  }, [profile.id, timeLimit, onUpdateTimeLimit, onSave])
